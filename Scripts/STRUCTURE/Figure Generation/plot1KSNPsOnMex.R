...
#plot samples
points(as.numeric(as.character(data$V8)), as.numeric(as.character(data$V7)), pch=as.numeric(as.character(data$V2)), col=as.character(data$V3), cex=.45) 
dev.off()

pdf("AllDataSmall21.pdf")
#plot Mexico
...
plot(states, lwd=0.5, add=TRUE)
#plot samples
points(as.numeric(as.character(data$V8)), as.numeric(as.character(data$V7)), pch=as.numeric(as.character(data$V2)), col=as.character(data$V3), cex=.6)
legend(list(x=-96,y=24), c("parvHC","parvAmb", "parvHyb","mexHC","mexAmb","mexHyb","maize"), col=c("blue2","cyan","purple1","firebrick4","firebrick1","purple1","gray38"), pch=c(0,0,0,2,2,2,1), lwd=2, lty=0, y.intersp=1.4)
dev.off()